...
EMBEDDING_COLOR_MAP = {
    ...
    for experiment, color in zip(
        EXPERIMENTS,
        plt.cm.managua(np.linspace(0.15, 0.85, len(EXPERIMENTS)))
    )
}
...
def plot_embedding_comparison(
    ...
    plot_data = (
        results[
            ["Experiment", "Train size", "Macro F1"]
        ]
    ...
    train_sizes = sorted(plot_data["Train size"].unique())
    ...
    if handles:
        ...
        print("Warning: no curves were created for the plot")
...
def save_results_table(results, dataset_directory):
    ...
    display_data.to_csv(csv_path, index=False)
    display_data.to_html(html_path, index=False)
...
def main():
    ...
    if len(sys.argv) != 2:
        ...
        print("Usage: python compare_embeddings.py <dataset_directory>")
        return
    ...
    print()
    print("=" * 80)
    print(f"EMBEDDING COMPARISON - {dataset_name}")
    print("=" * 80)

    # Load
    results = load_embedding_results(dataset_directory)
    ...
    # Save
    save_results_table(results, dataset_directory)

    # Check experiments
    print()
    print("Experiments found:", results["Experiment"].unique())
